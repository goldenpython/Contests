...
int main() {
	const int NO_POW_3 = 66;
	typedef BigInt<10000> BIGINT_TYPE;
	std::vector<BIGINT_TYPE> oVecPow3;
	oVecPow3.push_back(BIGINT_TYPE(1));
	for (int nLoop = 1; nLoop < NO_POW_3; nLoop++) {
		BIGINT_TYPE oPrevPow = oVecPow3[nLoop - 1];
		oPrevPow *= 3;
		oVecPow3.push_back(oPrevPow);
	}

	for (long long llN; (std::cin >> llN) && llN; ) {
		std::cout << "{";
		llN--;
		for (auto oItPow3 = oVecPow3.cbegin(); llN; ++oItPow3, llN >>= 1)
			if (llN & 0x1)
				std::cout << ' ' << *oItPow3 << ((llN > 1) ? "," : "");
